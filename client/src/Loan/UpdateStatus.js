...
function UpdateStatus({ onClose, row, onStatusUpdate }) {
  ...
  const sendSMS = async (payload) => {
    const smsBody = `Hello ${payload.firstName}, your loan application for ${payload.university} has been updated. Total Loan Amount: Rs.${payload.totalLoanAmount}.00, selected bank: ${payload.selectedBank}. Current Application Status: ${payload.status}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Call the sendSMS function to notify the student about the status update
    sendSMS({
      firstName: row.firstName, // Assuming you have this data in row
      university: row.university, // Assuming you have this data in row
      totalLoanAmount: row.totalLoanAmount, // Assuming you have this data in row
      selectedBank: row.selectedBank, // Assuming you have this data in row
      phoneNumber: row.phoneNumber, // Assuming you have this data in row
