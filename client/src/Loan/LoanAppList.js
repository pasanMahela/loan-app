...
function LoanAppList() {
    ...
    const fetchLoanApplications = async () => {
      try {
        ...
        const sortedApplications = response.data.sort((a, b) => 
          new Date(a.abroadDate) - new Date(b.abroadDate) // Sort by abroadDate in ascending order
      );
  ...
  const handleGenerateReport = () => {
    ...
    // Image hosted in the public folder
            ...
            <tbody>
              ${loanApplications.map(row => `
                <tr>
                  ...
                  <td>Rs.${row.totalLoanAmount}.00</td>
  ...
  <table className="w-full text-sm text-left text-gray-500">
    <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0">
      <tr>
        ...
        {/* Combine University and Program */}
        <th scope="col" className="px-6 py-3 whitespace-nowrap">University & Program</th>
        {/* Add Abroad Date */}
        <th scope="col" className="px-6 py-3 whitespace-nowrap">Abroad Date</th>
    ...
    <tbody>
      {filteredApplications.map((row) => (
        <tr key={row._id} className="bg-white border-b">
          ...
          <td className="px-6 py-4">
            Rs.{parseInt(row.totalLoanAmount).toLocaleString()}.00
          </td>
          {/* Combine University and Program in one column */}
          <td className="px-6 py-4">
            {row.university}
            <br />
          ...
          {/* Show Abroad Date */}
          <td className="px-6 py-4">
            {row.abroadDate ? new Date(row.abroadDate).toLocaleDateString() : 'N/A'}
          </td>
          <td className="px-6 py-4">
            <span className={
